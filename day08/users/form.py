...
class UserLoginForm(forms.Form):
    username = forms.CharField(required=True, error_messages={'required': '用户名为必填项'})
    password = forms.CharField(required=True, error_messages={'required': '密码为必填项'})
    ...
    def clean_username(self):
        if 'username' not in self.cleaned_data.keys():
            ...
            if len(username.strip()) > 16:
                raise forms.ValidationError('账号太长了')
            elif len(username.strip()) <= 2:
                raise forms.ValidationError('账号不能低于两个字符')
            else:
                return username
    ...
    def clean_password(self):
        if 'password' not in self.cleaned_data.keys():
            ...
            if len(password.strip()) > 20:
                ...
                raise forms.ValidationError('密码不能低于8个字符')
            else:
                return password
...
class UserModefyForm(forms.ModelForm):
    ...
    class Meta:
        model = UserProfile
        fields = ['id', 'phone', 'age', 'sex', 'is_active']
        # exclude = ['password']
